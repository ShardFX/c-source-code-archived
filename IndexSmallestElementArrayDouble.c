...
Write a function that returns the index of the smallest element in an array of double.
If the number of such elements is greater than 1, return the smallest element index.
...
void readInput(void){
    ...
    for(register size_t i = 0; i < size; i++){
        printf("Please enter the value of array[%d]: ", i);
        scanf("%lf", &array[i]);
    } // end for loop
...
int main(void){
    readInput(); // runs function indexOfSmallestElement()
    getchar(); // get character before exiting
    return EXIT_SUCCESS;
} // end function main
